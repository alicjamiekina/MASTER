...
{
//	RS485_FLAGS_SW.endofRX_SW = 0;
	if( endofRX_SW == 0  ){
    receiveData_SW();
	}
  //  RS485_FLAGS_SW.endofRX_SW=1;
	//if (   RS485_FLAGS_SW.endofTX_SW == 0 && endofTX_EF == 1){
        sendData_SW();
		//RS485_FLAGS_SW.endofTX_SW=1;
	//	RS485_FLAGS_SW.endofRX_SW=1;
	//} else if(  RS485_FLAGS_SW.endofTX_SW == 1){
	//	RS485_FLAGS_SW.endofstep_SW = 1;
	//	RS485_FLAGS_SW.endofTX_SW=0;
	//	RS485_FLAGS_SW.endofRX_SW=0;
	//	endofstep_EF=0;
	//}
}
...
static void receiveData_SW(void)
{
  ...
  if (cntEndOfRxTick_SW > RX_FRAME_LENGHT_SW)
    ...
    //  uint8_t crcSumOnMCU = 1;
    		  //HAL_CRC_Calculate(&hcrc, (uint32_t*)dataFromRx_SW, (RX_FRAME_LENGHT_SW - 2));
