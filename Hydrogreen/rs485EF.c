...
{
  	uint8_t crcSumOnMCU_EF = 0xFF;
  	uint8_t xbit, data1 =1;
	#define polynomial 0x7;

  	for(uint8_t l = 0; l < RX_FRAME_LENGHT_EF-1; l++){
  	uint8_t data = dataFromRx_EF[l];
  	xbit = data1 << 7;
  	for(uint8_t k = sizeof(RX_FRAME_LENGHT_EF-1)*8; k > 0;--k) // obliczanie wartosci najbardziej znaczacego bitu
  	{
  		...
  		if(data & xbit){
  			crcSumOnMCU_EF = crcSumOnMCU_EF ^polynomial;
  		}
  		xbit >>= 1;
  	}
  	}
...
//   endofTX_EF=1;
  // endofRX_EF=1;
	//} else if( endofTX_EF == 1){
		//endofstep_EF = 1;
	//     endofTX_EF=0;
	  //   endofRX_EF=0;
	  //   RS485_FLAGS_SW.endofstep_SW=0;
	//}
}
